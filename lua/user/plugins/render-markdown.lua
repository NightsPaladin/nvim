return {
  ...
  config = function()
    -- Use pcall to suppress the initialization error that ocurs on startup
    -- The error is harmless and rendering works correctly after initialization completes.
    ...
    -- If there was an error, silently ignore it on startup
    -- The plugin will work correctly once initialized
    if not ok and not string.match(err, "attempt to index a boolean value") then
      -- Only show errors that aren't the known startup race condition
      vim.notify("render-markdown.nvim setup error: " .. err, vim.log.levels.ERROR)
    end
  end,
  keys = {
    {
